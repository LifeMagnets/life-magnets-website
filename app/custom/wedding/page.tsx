...
export default function WeddingMagnetsPage() {
  return (
    <div>
      ...
      {/* Header */}
      <section style={{ background: 'linear-gradient(to bottom, #fdf2f0 0%, #f8dcd8 100%)', padding: '64px 16px 56px', textAlign: 'center' }}>
        <div style={{ maxWidth: 640, margin: '0 auto' }}>
          <Link href="/custom" style={{ display: 'inline-block', marginBottom: 24, color: '#c8675d', fontWeight: 700, fontSize: '0.9rem', textDecoration: 'none' }}>
            ← Back to Custom Orders
          </Link>
          <div style={{ fontSize: '0.8rem', fontWeight: 800, letterSpacing: '0.14em', textTransform: 'uppercase', color: '#c8675d', marginBottom: 14 }}>Custom Magnets</div>
      ...
      <section style={{ background: '#fafaf8', padding: '56px 16px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(380px, 1fr))', gap: 20 }}>
            {designTypes.map(d => (
              <div key={d.label} style={{ background: 'white', border: '2px solid rgba(200,103,93,0.15)', borderRadius: 16, padding: '28px 28px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
                  <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#c8675d', flexShrink: 0 }} />
                  <h3 style={{ fontSize: '1rem', fontWeight: 800, color: '#1a3a40', margin: 0 }}>{d.label}</h3>
                </div>
                <p style={{ fontSize: '0.9rem', color: '#6a8e95', lineHeight: 1.65, margin: 0 }}>{d.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ maxWidth: 640, margin: '0 auto', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 20 }}>
            {[
              ...
              <div key={s.label} style={{ padding: '10px 18px', background: '#fdf2f0', border: '2px solid rgba(200,103,93,0.25)', borderRadius: 10, fontWeight: 700, color: '#c8675d', fontSize: '0.88rem' }}>
                {s.label}
              </div>
            ))}
          </div>
          <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 36 }}>
            {[
              ...
              <div key={t.label} style={{ padding: '8px 16px', background: '#fafaf8', border: '1px solid #e8d0c8', borderRadius: 8, fontSize: '0.82rem', color: '#6a8e95', fontWeight: 600 }}>
      ...
      <section style={{ background: '#fafaf8', padding: '48px 16px 72px' }}>
        <div style={{ maxWidth: 800, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 24 }}>
          ...
          {/* Order online */}
          <div style={{ background: 'white', border: '2px solid rgba(200,103,93,0.2)', borderRadius: 18, padding: '32px 28px', display: 'flex', flexDirection: 'column', gap: 16 }}>
            <h3 style={{ fontSize: '1.15rem', fontWeight: 800, color: '#1a3a40', margin: 0 }}>Order Online</h3>
            <p style={{ fontSize: '0.9rem', color: '#6a8e95', lineHeight: 1.65, margin: 0 }}>
              Upload your photo, choose your size and quantity, and place your order directly. We'll handcraft and ship them to you.
            </p>
            <Link href="/custom/build?category=wedding" style={{ display: 'block', textAlign: 'center', padding: '14px 0', background: '#c8675d', color: 'white', fontWeight: 700, fontSize: '0.95rem', textDecoration: 'none', borderRadius: 11, marginTop: 'auto' }}>
              Start Your Wedding Order →
            </Link>
          </div>

          {/* Booth package */}
          <div style={{ background: 'linear-gradient(135deg, #1a3a40 0%, #0d2a30 100%)', border: '2px solid rgba(200,103,93,0.4)', borderRadius: 18, padding: '32px 28px', display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ display: 'inline-block', background: '#c8675d', color: 'white', fontSize: '0.72rem', fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase', padding: '4px 10px', borderRadius: 6, alignSelf: 'flex-start' }}>
              Live Experience
            </div>
            ...
            <Link href="/events/weddings" style={{ display: 'block', textAlign: 'center', padding: '14px 0', background: '#c8675d', color: 'white', fontWeight: 700, fontSize: '0.95rem', textDecoration: 'none', borderRadius: 11, marginTop: 'auto' }}>
              See Booth Packages →
            </Link>
          </div>
