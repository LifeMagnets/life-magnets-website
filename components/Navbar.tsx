...
export default function Navbar() {
  ...
  return (
    <nav style={{ background: 'white', borderBottom: '2px solid #26c6da', position: 'sticky', top: 0, zIndex: 50 }}>
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 64 }}>
        ...
        <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none' }}>
          ...
          <span style={{ fontWeight: 800, fontSize: '1.1rem', color: '#0097a7', letterSpacing: '-0.01em' }}>Life Magnets Co.</span>
        </Link>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }} className="desktop-nav">
          ...
          <Link href="/custom" className="btn" style={{ marginLeft: 8, padding: '9px 20px', background: '#0097a7', color: 'white', fontWeight: 700, fontSize: '0.92rem', textDecoration: 'none', borderRadius: 10 }}>
            Order Now
          </Link>
        </div>

        {/* Mobile hamburger */}
        <button onClick={() => setMenuOpen(o => !o)} style={{ display: 'none', background: 'none', border: 'none', fontSize: '1.5rem', cursor: 'pointer', color: '#0097a7' }} className="mobile-menu-btn">
          {menuOpen ? '✕' : '☰'}
        </button>
      </div>
      ...
      {menuOpen && (
        <div style={{ borderTop: '1px solid #b2ebf2', background: 'white', padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          <Link href="/custom" onClick={() => setMenuOpen(false)} className="btn" style={{ margin: '8px 0 4px', padding: '12px', background: '#0097a7', color: 'white', fontWeight: 700, textDecoration: 'none', borderRadius: 10, textAlign: 'center' }}>
            Order Now
          </Link>
        </div>
      )}
